...
{
    public abstract class DefaultHelpPrinterTest
    {
        ...
        private readonly DefaultHelpPrinter _sut;

        protected DefaultHelpPrinterTest(Dialect dialect, string shortPrefix, string longPrefix, string optionValueSeparator)
        {
            _dialect = dialect;
        ...
        public void PrintHelp_ShouldPrintBasicInfoWithDefaultName()
        {
            var commandConfig = new CommandConfig(
                    new CliConfig(
                        ...
                        _sut,
                        true,
                        new IArgumentConverter[0]
                    ),
                    null,
                    false,
                    new List<string>(),
                    new List<CommandConfigurator>(),
                    null,
                    new List<PrefixedDefaultValueArgument>
                    {
                        new FlagBuilder().Abr("h").Name("help").Description("Prints this help").Build()
                    },
                    new List<Parameter>(),
                    null,
                    (args, output) => 0
